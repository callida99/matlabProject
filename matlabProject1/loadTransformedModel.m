function [ mesh ] = loadTransformedModel( file, aspect )
%LOADMODEL loads a ply file and transforms the vertices to clip space
    ...
    modelM =  [ 1 0 0 0;
                ...
                0 0 0 1];
    viewM = eye(4);
    projM = perspectiveProj(90, aspect, 0.1, 10);
...
function [ proj ] = perspectiveProj( fov, aspect, nearZ, farZ )
%PERSPECTIVEPROJ creates a perspective projection matrix

    tanHalf = tand(fov/2);
    FN1 = -(nearZ + farZ)/(farZ - nearZ);
    FN2 = (-2*farZ*nearZ)/(farZ - nearZ);
    proj = [1/(tanHalf*aspect)  0           0       0;
            0                   1/tanHalf   0       0;
            0                   0           FN1     FN2;
            0                   0           -1      0];


function [vertex, color, face] = readPly(filename)
